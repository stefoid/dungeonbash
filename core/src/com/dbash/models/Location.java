...
public class Location {
	...
	public void addCentralLight(DungeonPosition thePosition) {
		...
		map.addLight(new Light(torchPosition, 1, Light.CENTRAL_TORCH_STRENGTH, true), false);
		torchPosition.y--;
		map.addLight(new Light(torchPosition, Light.WALL_TORCH_RANGE-1, Light.CENTRAL_TORCH_STRENGTH, true), false);
		torchPosition.y += 2;
		map.addLight(new Light(torchPosition, Light.WALL_TORCH_RANGE-1, Light.CENTRAL_TORCH_STRENGTH, true), false);
		torchPosition.y--;
		torchPosition.x--;
	...
	public void addTorch(TorchType torch) {
		...
		if (torch == TorchType.CENTRAL || (torch == TorchType.INVISIBLE)) {
			...
			torchPosition.y--;
			map.addLight(new Light(torchPosition, Light.WALL_TORCH_RANGE, Light.WALL_TORCH_STRENGTH, true), false);
	...
	private void setDiscoveredArea() {
		for (int h=x-1;h <= (x+1); h++) {
			for (int v=y-1;v <= (y+1); v++) {
				map.safeLocation(h, v).isDiscovered = true;
			}
		}
	}
