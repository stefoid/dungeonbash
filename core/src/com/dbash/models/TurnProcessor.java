...
public class TurnProcessor implements IPresenterTurnState {
	...
	private List<Character> createRandomCharacters() {
		...
		if (L.TESTCHARS) {
			theChars.clear();
			p.level = 20;
			theChars.add(new Character(Creature.getIdForName("spawn"), p, 1, dungeonEvents, dungeonQuery, this));
			theChars.add(new Character(Creature.getIdForName("living flame"), p, 2, dungeonEvents, dungeonQuery, this));
