...
public class TurnProcessor implements IPresenterTurnState {
	...
	private List<Character> createRandomCharacters() {
		...
		theChars.clear();
		p.level = 20;
		theChars.add(new Character(Creature.getIdForName("wizard"), p, 1,
				dungeonEvents, dungeonQuery, this));
		theChars.add(new Character(Creature.getIdForName("minotaur"), p, 2,
				dungeonEvents, dungeonQuery, this));
		theChars.add(new Character(Creature.getIdForName("web shooter"), p, 3,
				dungeonEvents, dungeonQuery, this));
		Creature c = theChars.get(0);
		c.addAbility(new Ability(Ability.getIdForName("wand: smashing"), null,
				20, dungeonEvents, dungeonQuery), null);
	...
	public Character getCurrentLeader() {
		...
		// else {
		// if (savedLeader != null) {
		// // If we are going to re-instate leader mode, is the leader alive and
		// still on hte map?
