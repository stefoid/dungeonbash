...
public class TurnProcessor implements IPresenterTurnState {
	...
	private List<Character> createRandomCharacters() {
		...
		if (L.TESTCHARS) {
			theChars.clear();
			p.level = 20;
			theChars.add(new Character(Creature.getIdForName("cave lion"), p, 1, dungeonEvents, dungeonQuery, this));
			theChars.add(new Character(Creature.getIdForName("living eye"), p, 2, dungeonEvents, dungeonQuery, this));
			theChars.add(new Character(Creature.getIdForName("blob demon"), p, 3, dungeonEvents, dungeonQuery, this));
